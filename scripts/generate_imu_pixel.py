"""
Generate Synthetic IMU dari 2D keypoint pixel (fixed camera approach)
Input : JSON OpenPose (2D pixel)
Output: /workspace/awal/synthetic_imu/{video_name}_imu.npy
Shape output: (N, 3) - [ax_pixel, ay_pixel, magnitude]
"""
    ...
    for f in files:
        ...
        if len(data['people']) > 0:
            ...
            if rc < CONF_THRESHOLD and lc < CONF_THRESHOLD:
                ...
                mh.append(kp[8, :2].tolist())
            else:
                mh.append(((kp[8, :2] + kp[11, :2]) / 2).tolist())
        else:
            mh.append([np.nan, np.nan])
    return np.array(mh)
...
def compute_acceleration(positions, dt):
    ...
    # Tambahkan magnitude sebagai kolom ke-3
    mag = np.linalg.norm(accel, axis=1, keepdims=True)
    return np.hstack([accel, mag])  # (N, 3)
...
for count, folder in enumerate(video_folders, start=1):
    ...
    mag = accel[:, 2]  # magnitude sudah di kolom ke-3
