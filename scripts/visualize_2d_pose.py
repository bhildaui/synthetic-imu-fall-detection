"""
Visualisasi 2D Pose MediaPipe - overlay keypoint di atas frame asli
Input : Frame PNG + JSON keypoint MediaPipe
Output: Video MP4 dengan skeleton overlay
"""
...
# Koneksi skeleton Detectron COCO 17 keypoint
SKELETON = [
    (0, 1), (0, 2),           # Nose - LEye, Nose - REye
    (1, 3), (2, 4),           # LEye - LEar, REye - REar
    (0, 5), (0, 6),           # Nose - LShoulder, Nose - RShoulder
...
COLORS = {
    'head':  (255, 200, 0),    # kuning
    'upper': (0, 200, 255),    # cyan
    'lower': (0, 255, 100),    # hijau
}
...
def create_pose_video(video_name, label):
    """Buat video 2D pose overlay"""
    frame_dir  = RGB_DIR / video_name / video_name
    json_dir   = JSON_DIR / video_name
    output_mp4 = OUT_DIR / f"2d_pose_{video_name}.mp4"
    ...
    for i in range(n):
        ...
        cv2.putText(frame, f"MediaPipe 2D Pose",
...
# Buat video untuk ADL dan Fall
print("Membuat video 2D pose MediaPipe...")
print("=" * 50)
